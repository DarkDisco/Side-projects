...
K=1              # Free particle wavenumber K equal to 1 for simplicity
...
xmax=400             #number of steps/ upper limit of range
x=5; y=[0,T,0.0]   # positive x position so that integration is in the negative direction;  y,y1,y2 
...
plt.xlabel('x [au]')
plt.ylabel('Psi(x) [au]')
someX, someY = 0, 0                  # Adds rectangle for barrier graphic on graph
currentAxis = plt.gca()
currentAxis.add_patch(Rectangle((someX - 0.0, someY - 20), 1, 40,
                      alpha=1, facecolor='r'))

plt.savefig("example of wavefunction2.pdf", bbox_inches="tight")  # saves graph
...
avg=sum(Tcoefficent)/len(Tcoefficent) # average transmission coefficent 
print avg
